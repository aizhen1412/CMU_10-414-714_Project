...
def compute_gradient_of_variables(output_tensor, out_grad):
    ...
    """
    # A map from node to a list of gradient contributions
    ...
    # Initialize gradients for all nodes to zero
    for node in reverse_topo_order:
        # Sum all gradient contributions to this node
        grads = node_to_output_grads_list.get(node, [])
        node.grad = sum(grads)  # vi

        if node.shape != node.grad.shape:
            node.grad = needle.ops.broadcast_to(node.grad, node.shape).detach()
...
def find_topo_sort(node_list: List[Value]) -> List[Value]:
    ...
    for node in node_list:  # iterate over all nodes
